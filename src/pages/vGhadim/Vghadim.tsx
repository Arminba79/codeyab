...
import { GetCourseOld } from '../../services/api';
...
function Vghadim() {
    ...
    bookMark.map((course) => { course.id })




    useEffect(() => {
        let data = GetCourseOld().then(courses => {
            setCourse(courses);
            console.log(data)
        })
        console.log(data)
    ...
    return (
        <Section className='items-center justify-between'>
            ...
            <Glasess className='w-11/12 min-h-9/12 max-h-9/12 xl:h-115 2xl:h-170 py-2  flex flex-col'>
                {/* title */}
                <div className='h-13 sm:h-15 flex justify-between px-5 items-center text-sm sm:text-md md:text-lg md:px-20 py-5  font-bold text-white '>
                    <BackButton />
                    <span>لیست دروس چارت قدیمی</span>
                </div>
                {/* courses list */}
